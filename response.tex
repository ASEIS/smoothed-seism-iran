...
\response{Thanks for the comment.  Based on explanation of GMPEs section (please see comment no.?) we decided to define a suite of GMPE and also provide the uncertainty. We consider the epistemic uncertainty, based on defining two different model (which considers the uncertainty in $b-value$ and $M_max$). In order to make it clear we added two GMPE suites (plus and minus one standard deviation) also we defined the uncertainty in $M_{max}$. This approach will cover the most of uncertainty in the study.}. 
...
\response{There are earthquakes out of the regions, mainly above the Caspian see. Since those area are not in our region of interest, and are fairly far from our region, we ignored them.}
