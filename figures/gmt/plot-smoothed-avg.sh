...
gmt makecpt -CcbcRdPu -Do -T0/1/0.05 > ${CPTFILE}
...
gmt grdmath ${GRDFILE2} ${GRDFILE1} ADD 0.5 MUL = ${GRDFILE}
...
gmt ps2raster ${OUTFILE} -F${FILEONE}-${FILETWO}.pdf -Tf -Au0.5c 

gmt grdimage ${GRDFILE} -R43.5/61.5/34/40 -JM5i -C${CPTFILE} -P -V > ${OUTFILE}

gmt ps2raster ${OUTFILE} -F${FILEONE}-${FILETWO}.png -Tg -Au
...
open -a Adobe\ Acrobat\ Pro.app ${FILEONE}-${FILETWO}.pdf
